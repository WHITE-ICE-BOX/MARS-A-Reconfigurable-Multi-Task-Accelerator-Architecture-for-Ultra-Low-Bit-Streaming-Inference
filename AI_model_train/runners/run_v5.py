"""
v5 sweep: 3x3 down + bias (8-bit Int8Bias) + signed QuantIdentity + scalar alpha.

Differs from v4 by enabling bias=True with Int8Bias quantizer on the down conv.
This is the missing ingredient that v1/v2 had (in QuantAdapter / MultiBranchAdapter)
which let b1 reach 73.72%; v3/v4 dropped it.

Bits processed in order [1, 2, 4, 8, 16, 32]. Reuses v1/v2 backbones from
claude/pretrained_backbones/.

Output:
  claude/paper_results_bitwidth/v5/
    experiments/Transfer_v5_b{N}_adapter_e200/
    logs/v5_b{N}.log
    results.csv
    plots/v5.png
"""
...
OUTPUT_ROOT = os.path.join(THIS_DIR, 'paper_results_bitwidth', 'v5')
...
def build_cmd(bit, args):
    ...
    exp_name = f"Transfer_v5_b{bit}_adapter_e{args.epochs}"
...
def plot_results(df, out_path):
    ...
    ax.plot(sub['bit'], sub['acc'], marker='o', linewidth=2,
            label='v5: 3x3 + bias(Int8) + signed + scalar α')
    ...
    ax.set_title('v5: 3x3 down + Int8Bias + signed + scalar α', fontsize=12)
...
def main():
    ...
    print(f"v5 sweep: bits={args.bits}, epochs={args.epochs}, "
    ...
    print(df)
    plot_results(df, os.path.join(PLOT_DIR, 'v5.png'))
