"""
v3_cross sweep: v3 k=3 (no bias = no RC) adapter x 6 bits x 3 datasets.
Adapter design: kernel=3, ReLU/sign, per-channel alpha, mid_basis='out',
M=1, NO bias (= no RC). Same design family as Table 1 SVHN baseline.

3 datasets x 6 bits x 1 mode = 18 cells, M=1, 200ep.

Output:
  claude/paper_results_bitwidth/v_v3_cross/
    experiments/Transfer_v3cross_{dataset}_b{N}_e200/
    logs/{dataset}_b{N}.log
    results.csv
"""
...
OUTPUT_ROOT = os.path.join(THIS_DIR, 'paper_results_bitwidth', 'v_v3_cross')
...
def build_cmd(dataset, bit, args):
    ...
    exp_name = f"Transfer_v3cross_{dataset}_b{bit}_e{args.epochs}"
    ...
    cmd = [
        ...
        '--num_branches', '1',
        '--adapter_bit_width', str(bit),     # matched-bit
        '--adapter_kernel', '3',             # 3x3 (v3 k=3 design)
        '--adapter_act', 'relu',             # 1-bit auto-degrades to sign
        '--adapter_alpha', 'per_channel',
        '--adapter_mid_basis', 'out',
        '--no_rc',
        # NO --adapter_bias (= no RC)
    ]
    return label, cmd
...
def main():
    ...
    print(f"v_v3_cross sweep: 3 datasets x 6 bits = 18 cells (no RC, single-branch), "
