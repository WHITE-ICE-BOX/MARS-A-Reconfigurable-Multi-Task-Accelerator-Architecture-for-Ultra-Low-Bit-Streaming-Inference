...
def build_full_ft(source, target, bit, args, exp_dir):
    ...
    exp_name = f"Transfer_{source.lower()}2oCAbits_{target}_b{bit}_full_ft_e{args.epochs}"
...
def build_adapter_norc(source, target, bit, args, exp_dir):
    bp = os.path.join(BACKBONE_DIR, f'{source.lower()}_{bit}w{bit}a.tar')
    exp_name = f"Transfer_{source.lower()}2oCAbits_{target}_b{bit}_adapter_norc_e{args.epochs}"
...
def main():
    ...
    output_root = os.path.join(THIS_DIR, 'paper_results_bitwidth',
                               f'{args.source.lower()}_configA_bits')
